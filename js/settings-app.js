...
export async function renderSettingsApp(user = window.currentUser) {
  ...
  content.innerHTML = `<div class="page-header"><h2><i class="fa fa-building-user"></i> Settings App · Client & Department</h2><button class="btn-primary btn-sm" onclick="openClientForm()"><i class="fa fa-plus"></i> Tambah Client</button></div><div id="settingsAppBody" class="card" style="padding:18px;text-align:center;"><i class="fa fa-spinner fa-spin"></i> Memuat...</div>`
...
async function loadSettingsApp() {
  ...
  if (error) { body.innerHTML = `<p class="text-danger">Gagal memuat client: ${error.message}</p>`; return }
...
window.saveClient = async function(id = '') {
  ...
  closeUserModal(); showToast('Client tersimpan.', 'success'); await loadSettingsApp()
}
