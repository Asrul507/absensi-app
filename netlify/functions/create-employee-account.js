...
async function readSupabaseResponse(res, defaultStatusCode = 500) {
  ...
  if (!res.ok) {
    ...
    const message = details.length ? details.join(' | ') : `Supabase error ${res.status}`
